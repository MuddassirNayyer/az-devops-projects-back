...
function getTheme(hour) {
  for (const key of ['latenight','dawn','morning','afternoon','evening','night']) {
    const t = THEMES[key]
    const [s, e] = t.hour
...
export default function Home() {
  ...
  const addLog = (msg, type = '') => setLogs(prev => [...prev, { msg, type, t: new Date().toLocaleTimeString() }])

  const totalRepos = data ? data.projects.reduce((s, p) => s + p.repos.length, 0) : 0
  const selectedRepos = data ? data.projects.filter(p => selected.has(p.id)).reduce((s, p) => s + p.repos.length, 0) : 0
  const pct = progress.total > 0 ? Math.round((progress.done / progress.total) * 100) : 0

  const toggleProject = (id) => setSelected(prev => {
  ...
  const startBackup = async () => {
    ...
    data.projects.forEach(proj => {
      if (selected.has(proj.id)) proj.repos.forEach(repo => repos.push({ proj, repo }))
    })
    setRunning(true)
    setLogs([])
    setRepoStatus({})
    ...
    setShowRepos(true)
    addLog(`Starting backup — ${repos.length} repo(s)`, 'info')

    let done = 0
    for (const { proj, repo } of repos) {
      setRepoStatus(prev => ({ ...prev, [repo.id]: 'downloading' }))
      const branch = repo.defaultBranch?.replace('refs/heads/', '') || 'main'
      const url = `/api/download?projectId=${proj.id}&repoId=${repo.id}&repoName=${encodeURIComponent(repo.name)}&projectName=${encodeURIComponent(proj.name)}&branch=${encodeURIComponent(branch)}`
      try {
        ...
        a.download = `${proj.name}_${repo.name}_${new Date().toISOString().slice(0,10)}.zip`
        document.body.appendChild(a); a.click()
        URL.revokeObjectURL(a.href); document.body.removeChild(a)
        setRepoStatus(prev => ({ ...prev, [repo.id]: 'done' }))
        addLog(`Done: ${repo.name}`, 'ok')
      } catch (e) {
        setRepoStatus(prev => ({ ...prev, [repo.id]: 'error' }))
        addLog(`Failed: ${repo.name} — ${e.message}`, 'err')
      }
      done++
      setProgress({ done, total: repos.length })
      await new Promise(res => setTimeout(res, 300))
    }
    addLog(`Backup complete. ${done}/${repos.length} repos downloaded.`, 'ok')
    setRunning(false)
  }
  ...
  return (
    <>
      <Head>
        ...
        <link href="https://fonts.googleapis.com/css2?family=Instrument+Sans:wght@400;500;600&family=Playfair+Display:ital@0;1&display=swap" rel="stylesheet" />
        <style>{`
      ...
      <div style={{
        ...
        background: T.bg, fontFamily: "'Instrument Sans', sans-serif",
        ...
        <div style={{
          ...
          background: 'radial-gradient(ellipse at 30% 20%, rgba(255,255,255,0.03) 0%, transparent 60%), radial-gradient(ellipse at 70% 80%, rgba(255,255,255,0.02) 0%, transparent 50%)',
          pointerEvents: 'none',
        }} />
          ...
          <div style={{ width: '100%', maxWidth: '560px' }}>
            ...
            <div className="fade1" style={{
              ...
              <span style={{
                ...
                {T.label} · {new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
              </span>
            </div>

            {/* Greeting */}
            <div className="fade2" style={{ textAlign: 'center', marginBottom: '12px' }}>
              <div style={{
              ...
              <div style={{
                ...
                fontSize: 'clamp(32px, 6vw, 52px)',
                fontWeight: '400',
                color: T.accent, lineHeight: '1.15',
                letterSpacing: '-0.01em',
            ...
            <div className="fade2" style={{
              ...
              fontSize: '15px', color: T.textMuted, letterSpacing: '0.01em',
            }}>
              ...
              <div className="fade3" style={{
                ...
                {[
                  ...
                  <div key={l} style={{
                    ...
                    <div style={{ fontSize: '11px', color: T.textMuted, textTransform: 'uppercase', letterSpacing: '0.08em', marginTop: '2px' }}>{l}</div>
                  </div>
                ))}
              </div>
            )}
            ...
            {running && (
              <div style={{ marginBottom: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: T.textMuted, marginBottom: '8px' }}>
                  <span>{progress.done} / {progress.total} repos</span>
                  <span>{pct}%</span>
                </div>
            ...
            {error && (
              <div className="fade3" style={{
                ...
                color: '#ffaaaa', fontSize: '13px',
              }}>
                <strong>Connection error:</strong> {error}<br />
                <span style={{ opacity: 0.7, fontSize: '12px', marginTop: '4px', display: 'block' }}>
            ...
            {loading && (
              <div className="fade3" style={{
                textAlign: 'center', color: T.textMuted, fontSize: '14px', marginBottom: '24px',
                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px',
              }}>
                <span className="spin" style={{ fontSize: '16px', color: T.accent }}>↻</span>
            ...
            {data && !loading && (
              <div className="fade4">
                ...
                >
                  {running ? (
                    <><span className="spin">↻</span> Downloading... ({progress.done}/{progress.total})</>
                  ) : (
                    <>
                ...
                <div style={{ textAlign: 'center', marginTop: '16px' }}>
                  <button
                    ...
                    style={{
                      ...
                      color: T.textMuted, fontSize: '13px', fontFamily: 'inherit',
            ...
            {data && showRepos && (
              <div style={{
                ...
                <div style={{
                  ...
                  <span style={{ fontSize: '12px', color: T.textMuted, textTransform: 'uppercase', letterSpacing: '0.08em' }}>Repositories</span>
                  <button onClick={toggleAll} style={{ background: 'none', border: 'none', cursor: 'pointer', color: T.accent, fontSize: '12px', fontFamily: 'inherit' }}>
                    {selected.size === data.projects.length ? 'Deselect all' : 'Select all'}
                  </button>
                </div>
                  ...
                  {data.projects.map(proj => {
                    ...
                    return (
                      <div key={proj.id} style={{ marginBottom: '4px' }}>
                        ...
                        >
                          <input type="checkbox" checked={isSel} onChange={() => toggleProject(proj.id)}
                            onClick={e => e.stopPropagation()}
                            style={{ accentColor: T.accent, width: '14px', height: '14px', cursor: 'pointer', flexShrink: 0 }}
                          />
                          <span style={{ flex: 1, fontSize: '13px', color: T.text, fontWeight: '500' }}>{proj.name}</span>
                          <span style={{ fontSize: '11px', color: T.textMuted }}>{proj.repos.length} repos</span>
                          {proj.repos.length > 0 && (
                            <button onClick={e => toggleExpand(proj.id, e)}
                              style={{ background: 'none', border: 'none', color: T.textMuted, cursor: 'pointer', fontSize: '10px', padding: '0 2px' }}>
                              {isExp ? '▲' : '▼'}
                            </button>
                          )}
                        </div>
              ...
              <div ref={logRef} style={{
                ...
                {logs.map((l, i) => (
                  <div key={i} style={{
                    ...
                    color: l.type === 'ok' ? '#6bcf7a' : l.type === 'err' ? '#ff7070' : l.type === 'info' ? T.accent : T.textMuted,
                  }}>
                    [{l.t}] {l.msg}
                  </div>
                ))}
            ...
            <div style={{
              ...
              fontSize: '12px', color: T.textMuted, opacity: 0.5,
            }}>
              {data?.org?.replace('https://dev.azure.com/', '') || 'Azure DevOps'} · Aquila360
            </div>
